"""
categorical_evaluate.py

Mirrors evaluate_model.py for the categorical gaze predictor.
Loads a checkpoint, generates synthetic gaze sequences by sampling from the
model's categorical distributions, then runs the same GazeEvaluator metrics
(fixation duration, saccade amplitude, main sequence, density KL, AUC, …).
...
Usage:
    python categorical_evaluate.py \
        ...
        --out_dir     /home/janhof/thesis/eval_categorical \
        --n_generate  50 \
        --seed_len    10 \
        --gen_len     128 \
        --temperature 1.0

Output layout is identical to evaluate_model.py:
    <out_dir>/
...
from kaamba.utils.gaze_eval import GazeEvaluator
...
def generate_sequences(
    model: GazeCategoricalPredictor,
    images: torch.Tensor,
    seed_len: int = 10,
    gen_len: int = 128,
    ...
    """
    Autoregressively sample gaze sequences from the categorical model.

    Starts with a uniform-random seed of length ``seed_len`` and extends to
    ``gen_len`` total steps by sampling from the predicted (x, y) categorical
    distributions at each step.

    Args:
        model:       GazeCategoricalPredictor (already on device, eval mode)
        images:      (N, 3, H, W) — one image per sequence to generate
        seed_len:    number of random seed steps before generation starts
        gen_len:     total sequence length to return (including seed)
        temperature: softmax temperature; >1 → more random, <1 → sharper
        device:      torch device string

    Returns:
        (N, gen_len, 2) numpy array of normalised [0, 1] gaze coordinates
    """
    model.eval()
    n_bins = model.n_bins
    B = images.shape[0]

    # Random seed in normalised [0, 1] gaze space
    seq = torch.rand(B, 2, seed_len, device=device)

    for _ in range(gen_len - seed_len):
        logits_x, logits_y = model(images, seq)  # (B, T_cur, n_bins)

        # Sample from the predicted distribution at the last time step
        lx = (logits_x[:, -1, :] / temperature).softmax(-1)  # (B, n_bins)
        ly = (logits_y[:, -1, :] / temperature).softmax(-1)

        x_bin = torch.multinomial(lx, 1).squeeze(1)  # (B,)
        y_bin = torch.multinomial(ly, 1).squeeze(1)

        # Map bin index to bin-centre coordinate in [0, 1]
        x_coord = (x_bin.float() + 0.5) / n_bins
        y_coord = (y_bin.float() + 0.5) / n_bins

        next_pt = torch.stack([x_coord, y_coord], dim=1).unsqueeze(2)  # (B, 2, 1)
        seq = torch.cat([seq, next_pt], dim=2)

    # Return only gen_len steps (drop nothing — seq is exactly gen_len long)
    return seq.permute(0, 2, 1).cpu().numpy()  # (B, gen_len, 2)


# ---------------------------------------------------------------------------
# Helpers: event extraction via pymovements  (identical to evaluate_model.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Per-stimulus evaluation  (identical to evaluate_model.py)
# ---------------------------------------------------------------------------
...
def evaluate_stimulus(
    ...
    real_fix_df: pl.DataFrame,
    real_sac_df: pl.DataFrame,
    fake_fix_df: pl.DataFrame,
    fake_sac_df: pl.DataFrame,
    ...
    if rh is not None and fh is not None:
        rhn, fhn = rh / rh.sum(), fh / fh.sum()
        direction = {
            "kl_divergence": float(stats.entropy(rhn + 1e-8, fhn + 1e-8)),
            **_ks(rh, fh),
        }
    else:
...
def run_evaluation(
    ...
    model = build_categorical_gaze_predictor(**model_config, verbose=False)
    model.load_state_dict(ckpt["model_state_dict"])
    model = model.to(device).eval()
    n_bins = model.n_bins
    print(
        f"[eval] Categorical model loaded  "
        f"({sum(p.numel() for p in model.parameters()):,} params, bins={n_bins})"
    )
    ...
    # Build stimulus → image path lookup from fileinfo
    ...
    for row in stim_fileinfo.iter_rows(named=True):
        ...
        if not img_path.is_absolute():
            img_path = Path(f"{root}/{dataset_name}/stimuli") / img_path
    ...
    for stim_name, gaze_list in tqdm(by_stimulus.items(), desc="Stimuli"):
        ...
        real_arr = np.stack(real_norm_seqs)
        ...
        if img_path is None or not img_path.exists():
            print(f"  [skip] {stim_name}: image not found")
            continue
        ...
        img_tensor = TF.to_tensor(TF.resize(img, [224, 224])).unsqueeze(0).to(device)
        ...
        for seq in fake_norm:
            px_seq = seq * np.array([scr_w_px, scr_h_px])
...
# ---------------------------------------------------------------------------
# Aggregation + report  (identical to evaluate_model.py)
# ---------------------------------------------------------------------------
...
def _build_report(all_results: Dict, aggregate: Dict, total_time: float) -> str:
    lines = [
        "=" * 70,
        "CATEGORICAL GAZE EVALUATION REPORT",
...
# ---------------------------------------------------------------------------
# Comparison plot  (identical to evaluate_model.py)
# ---------------------------------------------------------------------------
...
def plot_best_worst_comparison(
    ...
    plot_cache: Dict,
    out_path: "str | Path",
    score_metric: tuple = ("classifier_auc", "auc"),
    n_scanpaths: int = 8,
    density_grid: int = 32,
    ...
    if len(ranked) < 2:
        print("[plot] need at least 2 stimuli — skipping")
        return
    ...
    def _draw_row(axes_row, cache, stim_name, metrics, row_label, score):
        real_seqs = cache["real_seqs"]
        fake_seqs = cache["fake_seqs"]
        ...
        try:
            ax_img.imshow(Image.open(img_path).convert("RGB"), aspect="auto")
        except Exception:
            ax_img.text(
                0.5,
                0.5,
        ...
        ax_img.set_yticks([])
        ax_img.set_title(
            f"{row_label}\n{stim_name[:28]}", fontsize=8, loc="left", pad=4
        )
    ...
    fig.suptitle(
        f"Categorical gaze evaluation — best vs worst matching stimulus\n"
        f"Ranked by {section.replace('_', ' ')} {key}  "
...
def _parse():
    p = argparse.ArgumentParser(
        description="Evaluate GazeCategoricalPredictor against real data"
    )
    p.add_argument("--checkpoint", required=True)
    p.add_argument("--dataset", required=True)
    p.add_argument("--root", required=True)
    p.add_argument("--out_dir", default="eval_results_categorical")
    p.add_argument("--subjects", nargs="*", default=None)
    p.add_argument("--stimuli", nargs="*", default=None)
    p.add_argument("--trial_ids", nargs="*", default=None)
    p.add_argument("--n_generate", type=int, default=50)
    p.add_argument("--seed_len", type=int, default=10)
    p.add_argument("--gen_len", type=int, default=128)
    p.add_argument("--temperature", type=float, default=1.0)
    p.add_argument("--vel_threshold", type=float, default=30.0)
    p.add_argument("--min_fix_dur", type=int, default=10)
    p.add_argument(
        "--vel_method",
        default="fivepoint",
...
def test():
    run_evaluation(
        checkpoint_path="/home/janhof/thesis/logs/runs_categorical/trial_0000/checkpoints/best_model.pt",
        dataset_name="mcfw-gaze",
        root="/home/janhof/thesis/data",
        out_dir="/home/janhof/thesis/eval_results_categorical",
