...
def _create_stimuli_dataframe(img_with_paths: str, n_pages=23) -> pl.DataFrame:
    ...
    stimulus_cols = np.array(
        [1, 2, 3, 4, 6, 7, 8, 9, 10, 11, 12, 13] * n_pages
    ).flatten()
    # reshape pages
    pages_long = df.select(text_cols).unpivot().rename({"value": "text"})

    # reshape image names
    imgs_long = df.select(img_cols).unpivot().rename({"value": "img_name"})
    ...
    stimuli = pl.DataFrame(
        {
            ...
            "page": np.array(
                [[f"page_{i + 1}"] * df.height for i in range(n_pages)]
            ).flatten(),
            "stimulus_id": stimulus_cols,
        }
    )
    stimuli = stimuli.drop_nulls().cast(
        {"stimulus_id": pl.String}
    )  # remove all non text pages (questions and familiarity ratings)
    return stimuli


def _create_eyetracking_dataframe(
    data_folder_regex, et_data_to_include=ET_DATA_TO_INCLUDE
) -> pl.lazyframe:
    ...
    eyetracking = pl.scan_csv(
        data_folder_regex, include_file_paths="file_path"
    ).with_columns(
        pl.col("file_path")
    ...
    eyetracking = eyetracking.group_by(
        ["page", "file_path", "participant_id", "stimulus_id"]
    ).agg(pl.col("data"))
    return eyetracking
...
def create_mp_metadata(
    img_with_paths: str,
    out_dir: dir,
    data_regex="data/**/raw_data/*.csv",
    data_to_include=META_DATA_TO_INCLUDE,
    datacollection_name="Goettingen",
) -> None:
    ...
    combined = eyetracking.join(stimuli.lazy(), on=["page", "stimulus_id"], how="left")
    combined = combined.drop_nulls().select(data_to_include)

    combined.sink_ndjson(out_dir / f"metadata_{datacollection_name}.jsonl")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create metadata for MultiplEYE dataset"
    )
    parser.add_argument(
        "--img_with_paths",
        type=str,
        default="C:/Users/saphi/PycharmProjects/thesis/data/MultiplEYE_DE_DE_Goettingen_1_2026/stimuli_MultiplEYE_DE_DE_Goettingen_1_2026/multipleye_stimuli_experiment_de_de_1_with_img_paths.csv",
        help="Path to the CSV file containing image paths and text",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="C:/Users/saphi/PycharmProjects/thesis/kaamba_repo/src/kaamba/kaamba_dataset/stimuli/Goettingen",
        help="Directory to save the output metadata",
    )
    parser.add_argument(
        "--data_regex",
        type=str,
        default="C:/Users/saphi/PycharmProjects/thesis/data/MultiplEYE_DE_DE_Goettingen_1_2026/**/raw_data/*.csv",
        help="Regex pattern to match eyetracking data files",
    )
    parser.add_argument(
        "--data_to_include",
        nargs="+",
        default=META_DATA_TO_INCLUDE,
    ...
    create_mp_metadata(
        args.img_with_paths, Path(args.out_dir), args.data_regex, args.data_to_include
    )
